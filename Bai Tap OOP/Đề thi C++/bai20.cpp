...
/*
    ...
    số phức (tối đa 10 phần tử). Sắp xếp lại danh sách đã nhập theo trật tự giảm dần của 
    module.
...
float SoPhuc2::TinhMuDunSoPhuc(SoPhuc2 sp) {
    sp.a = abs(sp.a);
    sp.b = abs(sp.b);
    return (float)sqrt((pow(sp.a, 2) + pow(sp.b, 2)));
}

bool SoPhuc2::operator = (SoPhuc2 &sp1) {
    return (float)sqrt(pow(this->a, 2) + pow(this->b, 2)) == (float)sqrt(pow(sp1.a, 2) + pow(sp1.b, 2));
}

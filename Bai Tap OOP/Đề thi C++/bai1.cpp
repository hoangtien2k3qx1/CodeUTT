...
/*
    Câu 1: Viết chương trình thực hiện các yêu cầu sau: 
        ...
        2. Xây dựng các phương thức: nhập, xuất, tính tổng điểm một đối tượng thí sinh. Xây dựng p hương 
        thức khởi tạo không đối và có đối
        3. Nhập vào n thí sinh. In ra màn hình thông tin của thí sinh có tổng điểm cao nhất 
        (yêu cầu sử dụng biến con trỏ để cấp phát và giải phóng bộ nhớ).
        4. Sắp xếp sinh viên tăng dần theo hoten
            Các h 1: dùng getterhoten
            Cách 2: dùng hàm bạn
        5. TÌm kiếm sinh viên theo họ và tên (vd: tìm sv tên Mai)
...
public: 
    SinhVien() {                                        // Constructor không đối số
        this->masv = ""; 
        this->hoten = "";
        this->diemtoan = this->diemly = this->diemhoa = 0;
    }

    SinhVien(string, string, float, float, float) {     // Constructor có đối số
        this->masv = masv;  
        this->hoten = hoten;
    ...
    string getHoten() {  // "hoang anh tien";
        return this->hoten;
    }
    ...
    void NhapThongTinSinhVien(SinhVien* , int); // Function Prototype 
    void XuatThongTin(SinhVien);
    void XuatThongTinSinhVien(SinhVien*, int); 
